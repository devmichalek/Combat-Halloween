...
int Vegetation::strToInt(string s)
{
    bool m=false;
    int tmp=0;
    unsigned i=0;
...
unsigned Vegetation::getDistance( int v1, int v2 )
{
	if( v1 < 0 )
		v1 = -v1;
...
void Vegetation::positioning( vector < Block* > blocks )
{
	for( unsigned i = 0; i < blocks.size(); i++ )
	{
		bool success = false;
	...
	{
		printf( "%d %d %d\n", this->blocks[ i ]->nr, this->blocks[ i ]->x, this->blocks[ i ]->y );
	}
	*/
}

void Vegetation::moveX( sf::Uint8 direction, float vel )
{
	if( direction == 1 )
	{
		for( unsigned i = 0; i < blocks.size(); i++ )
...
int Vegetation::backToGrass( int add )
{
	for( unsigned i = 0; i < blocks.size(); i++ )
	{
		blocks[ i ]->x += add;
