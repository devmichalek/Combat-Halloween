...
void Golem::matchX( int x, int w, int y, int h )
{
	if( state == 2 )
	{
		// printf( "%d %d   %d %d\n", y, y+h, sprite[ 1 ].getY(), sprite[ 1 ].getY() + sprite[ 1 ].getHeight() );
...
int Golem::getX()
{
	return sprite[ 1 ].getX();
}

int Golem::getY()
{
	return sprite[ 1 ].getY();
}

int Golem::getW()
{
	return sprite[ 1 ].getWidth();
}

int Golem::getH()
{
	return sprite[ 1 ].getHeight();
}
