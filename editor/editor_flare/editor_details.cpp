...
void Editor_details::saveCurrentName()
{
	// remove spaces.
	string newName = "";
	int state = 0;
	...
	// From the beggining.
	for( unsigned i = 0; i < nameStr.size(); i++ )
	{
		if( state == 0 && nameStr[ i ] != ' ' )
		{
			state = 1;
		}
	...
	// From the end.
	for( int i = nameStr.size() -1; i >= 0 ; i-- )
	{
		if( state == 0 && nameStr[ i ] != ' ' )
		{
			state = 1;
...
void Editor_details::setWritten()
{
	...
	if( nameStr.size() == 0 )	arrow.setPosition( name.getX() -5, name.getY() );
	else						arrow.setPosition( name.getRight(), name.getY() );
}
